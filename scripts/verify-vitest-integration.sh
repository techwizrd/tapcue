...
FIXTURE_DIR="$ROOT_DIR/tests/runner-fixtures/vitest-json"
...
if [[ $runner_status -eq 0 ]]; then
  echo "Expected failing status from vitest fixture."
  exit 1
fi

if ! grep -q '"numFailedTests"' "$tmp_file"; then
  echo "Expected Vitest JSON summary in output."
  exit 1
fi
...
if [[ $tapcue_status -eq 0 ]]; then
  echo "Expected tapcue to report failure for failing vitest fixture."
  exit 1
fi
